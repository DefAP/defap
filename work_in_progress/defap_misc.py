...
def format_dopants(dopants_lst):
    ...
    for dopant_index, dopant in enumerate(dopants_lst):
        ...
        if int(dopant[3]) == 1 or int(dopant[3]) == 2:
        ...
        # calc chem pot from partial pressure
        elif int(dopant[3]) == 3 or int(dopant[3]) == 4:
            individual_dopant_dict["log_PP_atm"] = float(dopant[4])

            # count number of dopant partial pressures being looped over for defect phase task
            if int(dopant[3]) == 4:
