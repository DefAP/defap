...
class ReadInputData:
    ...
    def __init__(self, seedname: str):
        ...
        self.simplify_shape = False
        self.shape_tolerance = None
    ...
    def read_input_file(self):
        ...
        self.input_data = {
            ...
            "x_variable": 0,  # plot as function of loop
            ...
            "simplify_shape": False, "shape_tolerance": None
        }
        ...
        with open(input_file, "r") as f:
            ...
            for line_index, line in enumerate(lines):
                ...
                elif variable == "iterator":
                    self.input_data[variable] = float(line[2])
                    self.iterator = float(line[2])

                elif variable == "loop_y":
                    self.input_data[variable] = float(line[2])
                    self.loop_y = float(line[2])

                elif variable == "min_value_y":
                    self.input_data[variable] = float(line[2])
                    self.min_value_y = float(line[2])

                elif variable == "max_value_y":
                    self.input_data[variable] = float(line[2])
                    self.max_value_y = float(line[2])

                elif variable == "iterator_y":
                    self.input_data[variable] = float(line[2])
                    self.iterator_y = float(line[2])

                elif variable == "x_variable":
                    self.input_data[variable] = int(line[2])
                ...
                elif variable == "bandgap":
                    self.input_data[variable] = float(line[2])
                    self.bandgap = float(line[2])

                elif variable == "chem_pot_method":
                    self.input_data[variable] = line[2].lower()
                    self.chem_pot_method = line[2].lower()

                elif variable == "art_dopant_conc":
                    self.input_data[variable] = float(line[2])
                    self.art_dopant_conc = float(line[2])

                elif variable == "art_dopant_chg":
                    self.input_data[variable] = int(line[2])
                    self.art_dopant_chg = int(line[2])

                elif variable == "secondary_phases":
                    phases_lst = []
                ...
                elif variable == "real_gas":
                    self.input_data[variable] = int(line[2])
                    self.real_gas = int(line[2])

                elif variable == "electron_method":
                    self.input_data[variable] = line[2].lower()
                    self.electron_method = line[2].lower()

                    if line[2].lower() == "effective_masses":
                        self.electron_effective_masses = eval(line[3])

                elif variable == "hole_method":
                    self.input_data[variable] = line[2].lower()
                    self.hole_method = line[2].lower()

                    if line[2].lower() == "effective_masses":
                        self.hole_effective_masses = eval(line[3])

                elif variable == "conductionband":
                    self.input_data[variable] = float(line[2])
                    self.conductionband = float(line[2])

                elif variable == "valenceband":
                    self.input_data[variable] = float(line[2])
                    self.valenceband = float(line[2])

                elif variable == "fu_unit_cell":
                    self.input_data[variable] = int(line[2])
                    self.fu_unit_cell = int(line[2])

                elif variable == "volume_unit_cell":
                    self.input_data[variable] = float(line[2])
                    self.volume_unit_cell = float(line[2])

                elif variable == "stoichiometry":
                    self.input_data[variable] = int(line[2])
                    self.stoichiometry = int(line[2])

                elif variable == "defect_conc_method":
                    self.input_data[variable] = line[2].lower()
                    self.defect_conc_method = line[2].lower()

                elif variable == "concentration_units":
                    self.input_data[variable] = int(line[2])
                    self.conc_units = int(line[2])

                elif variable == "y_axis_min":
                    self.input_data[variable] = float(line[2])
                    self.y_axis_min = float(line[2])

                elif variable == "y_axis_max":
                    self.input_data[variable] = float(line[2])
                    self.y_axis_max = float(line[2])

                elif variable == "tolerance":
                    self.input_data[variable] = float(line[2])
                    self.tolerance = float(line[2])

                elif variable == "max_iteration":
                    self.input_data[variable] = float(line[2])
                    self.max_iteration = float(line[2])

                elif variable == "slsqp_dial":
                    self.input_data[variable] = float(line[2])
                    self.slsqp_dial = float(line[2])

                elif variable == "dopant_fit_convergence":
                    self.input_data[variable] = float(line[2])
                    self.dopant_convergence = float(line[2])

                elif variable == "charge_convergence":
                    self.input_data[variable] = float(line[2])
                    self.charge_convergence = float(line[2])

                elif variable == "colour_scheme":
                    self.input_data[variable] = int(line[2])
                ...
                elif variable == "coulombic_correction":
                    self.input_data[variable] = int(line[2])
                    self.coulombic_correction = int(line[2])

                elif variable == "tab_correction":
                ...
                elif variable == "dielectric_constant":
                    self.input_data[variable] = float(line[2])
                    self.dielectric_constant = float(line[2])

                elif variable == "screened_madelung":
                    self.input_data[variable] = float(line[2])
                ...
                elif variable == "conduction_band_limits":
                    self.input_data[variable] = line[2:]
                    self.conductionband_limits = line[2:]

                elif variable == "valence_band_limits":
                    self.input_data[variable] = line[2:]
                    self.valenceband_limits = line[2:]

                elif variable == "lattice_array":
                    lattice_array_lst = []
                    for i in range(3):
                        row = list(map(float, lines[line_index + i + 1]))
                        lattice_array_lst.append(row)

                    self.input_data["lattice_array"] = lattice_array_lst
                    self.lattice_array = np.array(lattice_array_lst)

                elif variable == "dielectric_array":
                    dielectric_array_lst = []
                    for i in range(3):
                        row = list(map(float, lines[line_index + i + 1]))
                        dielectric_array_lst.append(row)

                    self.input_data["dielectric_array"] = dielectric_array_lst
                    self.dielectric_array = np.array(dielectric_array_lst)

                elif variable == "simplify_shape" and line[2] in ['t', 'true', '1']:
                    self.input_data[variable] = True
                    self.simplify_shape = True

                elif variable == "shape_tolerance":
                    self.input_data[variable] = float(line[2])
                    self.shape_tolerance = float(line[2])
            ...
            if self.simplify_shape and self.shape_tolerance is None and "defect_phases" in self.tasks:
                self.shape_tolerance = 2 * min(self.iterator, self.iterator_y)
                self.input_data["shape_tolerance"] = 2 * min(self.iterator, self.iterator_y)
                    ...
                    if row_index == 0:
                        ...
                        if dopants:
                            for dopant_index, dopant in enumerate(dopants.keys()):
                                atoms_added_removed[dopant] = int(defect_row[7 + len(host_compound_elements) + dopant_index])
    ...
    def read_dos_data(self):
        ...
        if os.path.exists(f"{self.seedname}.dos"):
            ...
            self.dos_data = {"energies": None, "states": None}
