...
def main_wrapper(func):
    def wrapper():
        ...
        print("|                             |_|    |___/  |")
        print("|                                           |")
        print("|       Defect Analysis Package : 4.0       |")
...
def main():
    ...
    # calculate screened madelung
    if "madelung" in data.tasks:
        Tasks(data=data, seedname=seedname).madelung()

    # run formation_energy task
    if "formation_energy" in data.tasks:
        Tasks(data=data, seedname=seedname).formation_energies()
