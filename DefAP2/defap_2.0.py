...
# please contact: wdneilson@outlook.com             #
#                                                   #
#####################################################
#                                                   #
...
def header():
    ...
    print( "|                             |_|    |___/  |")
    print( "|                                           |")
    print( "|       Defect Analysis Package : 2.0       |")
...
def inputs(seedname):
    ...
    min_value_y = -40           #Minimum value used in loop2
    max_value_y = 0             #Maximum value using in loop2
    iterator_y = 1              #Iterator2 between minimum_y and maximum_y
    loop2 = 0                   #Property2 to loop over 0 = partial pressure, 1 = temperature, 2 = dopant conc, 3 = aritifical dopant conc
    ...
    accomodate='El'             # The dopant element that the accomodation mechanmism is being tested for. 
    ...
    for i in tasks:
        if i not in ['brouwer','energy','form_plots','autodisplay','stability','madelung','bibliography','group','phases','defect_phase','dopant']:
    ...
    if 'dopant' in tasks:
        if 'defect_phase' not in tasks:
            print("<!> 'The 'dopant' task has no effect without the 'defect_phase' task")
            exit()
        if accomodate == 'El':
            print("<!> 'The 'dopant' task requires dopant to be specified with 'Accomodate' in input file")
            exit()
        ...
        for i in tasks:
            if i in ['brouwer','energy','form_plots','autodisplay','stability','group','defect_phase']:
                ...
                if ('defect_phase' in tasks):
                    ...
                    if ('dopant' in tasks):
                         print("\n..> Also plotting the accomodation mechanism of", accomodate, file=f)
...
def gibbs_check(gibbs_data,constituents_name_list,chem_pot_method,seedname):

    gibbsfile = str(seedname)+".gibbs"
    outputfile = str(seedname)+".output"
    with open(outputfile, 'a') as f:
        print("\n>>> Performing checks on", gibbsfile,'\n',file=f)

        if chem_pot_method ==3:
            i = 0
            while i < len(constituents_name_list):
                constituent = constituents_name_list[i]
                gibbs_constituent = gibbs_data[0][i+2]
                if(gibbs_constituent != constituent):
                    print("<!> ERROR: There is a mismatch in the name for the constituent", constituent, "and the Gibbs energy", gibbs_constituent)
                    print("    Recommend you go back to ensure constituents occur in the same order in the .input and .gibbs files and spellings are identical")
                    exit()
                i+=1
            print("   Constituents are listed in same order in .input and .gibbs files",file=f)

def calc_gibbs(gibbs_data,temperature,constituents_name_list,chem_pot_method,host_energy,constituents,seedname,prnt):
    ...
    with open(outputfile, 'a') as f:
        ...
        while i < length:
            ...
            #Run a quick check to see whether temperature falls in the range of the data
            if(temperature < min_temp or temperature > max_temp):
                print("<!> WARNING Temperature is outside of range of Gibbs data provided\n")
            ...
            if prnt ==1:
                print("   | %7s | %19f |" % (system_name, final_gibbs),file=f)
                
            gibbs.append(final_gibbs*1)
        ...
        if chem_pot_method ==2:
            ...
            host_energy=gibbs[0]
            constituents[3] = gibbs[0]
        elif chem_pot_method == 3:

            host_energy=gibbs[0]
            i=0
            while i < (length-1):
                constituents[6*i+5]=gibbs[i+1]
                i+=1
        
    return(host_energy,constituents)
...
def calc_opt_chem_pot(b,loop,defects,dopants,chemical_potentials,number_of_defects,host_supercell,tab_correction,E_VBM,total_species,use_coul_correction,length,dielectric,v_M,bandgap,temperature,def_statistics,nu_e,condband,valband,electron_method,hole_method,fixed_e_conc,fixed_p_conc,art_dop_conc,art_dop_charge,charge_convergence,val_band_min,val_band_max,cond_band_min,cond_band_max,seedname,entropies, fu_uc,electron_mass_eff,hole_mass_eff,unit_vol,charged_sys, log_diff_conv,function_tol,maxiter_dop,environment,prog_meter,prog_bar,num_iter,real_gas,SLSQP_dial):         
    ...
    if len(position)==1:
        ...
        while fail ==1:
            ...
            (nu_e_final,concentrations,dopant_concentration_sums,fail) = calc_fermi_dopopt(b,loop,defects,defects_form,number_of_defects,bandgap,temperature,def_statistics,nu_e,condband,valband,electron_method,hole_method,fixed_e_conc,fixed_p_conc,art_dop_conc,art_dop_charge,charge_convergence,val_band_min,val_band_max,cond_band_min,cond_band_max,seedname,entropies, fu_uc,electron_mass_eff,hole_mass_eff,unit_vol,charged_sys,number_dopants,bnds,dp_list,optimiser,log_diff_conv,function_tol,maxiter_dop,environment,prog_meter,prog_bar,num_iter,real_gas,x)
            if fail ==1:                                                 
                i +=0.05        
    ...
    #Multiple dopant elements to optimise
        ...
        while completion < 1:
            ...
            #Impose constraints
            cons = ({'type':'ineq','fun':constraint,'args':[ target_conc_list, b,loop,number_of_defects,host_supercell,tab_correction,E_VBM,total_species,use_coul_correction,length,dielectric,v_M,bandgap,temperature,def_statistics,nu_e,condband,valband,electron_method,hole_method,fixed_e_conc,fixed_p_conc,art_dop_conc,art_dop_charge,charge_convergence,val_band_min,val_band_max,cond_band_min,cond_band_max,seedname,entropies, fu_uc,electron_mass_eff,hole_mass_eff,unit_vol,charged_sys,number_dopants,position,key_list,bnds,dp_list,optimiser,environment,prog_meter,prog_bar,num_iter,log_diff_conv,function_tol,maxiter_dop,real_gas] })

            #Minimise function. (Minimising the difference between each dopant concentration and its target)
            sol = minimize(calc_opt_chem_pot_multidim, x0,args= (target_conc_list, b,loop,number_of_defects,host_supercell,tab_correction,E_VBM,total_species,use_coul_correction,length,dielectric,v_M,bandgap,temperature,def_statistics,nu_e,condband,valband,electron_method,hole_method,fixed_e_conc,fixed_p_conc,art_dop_conc,art_dop_charge,charge_convergence,val_band_min,val_band_max,cond_band_min,cond_band_max,seedname,entropies, fu_uc,electron_mass_eff,hole_mass_eff,unit_vol,charged_sys,number_dopants,position,key_list,bnds,dp_list,optimiser,environment,prog_meter,prog_bar,num_iter,log_diff_conv,function_tol,maxiter_dop,real_gas,iteration_relaunch), method = 'SLSQP',bounds =bnds,constraints= cons ,options={'ftol':function_tol,'disp':False,'maxiter':maxiter_dop})

            relaunch_triggered = globals()['relaunch_triggered']
     
            if relaunch_triggered == 1:
                           
                chemical_potentials, dopants ,iteration_relaunch, prior_fail_con=relaunch(b,loop,defects,dopants,chemical_potentials,number_of_defects,host_supercell,tab_correction,E_VBM,total_species,use_coul_correction,length,dielectric,v_M,bandgap,temperature,def_statistics,nu_e,condband,valband,electron_method,hole_method,fixed_e_conc,fixed_p_conc,art_dop_conc,art_dop_charge,charge_convergence,val_band_min,val_band_max,cond_band_min,cond_band_max,seedname,entropies, fu_uc,electron_mass_eff,hole_mass_eff,unit_vol,charged_sys, log_diff_conv,function_tol,maxiter_dop,environment,prog_meter,prog_bar,num_iter,real_gas,number_dopants,x0, bnds,dp_list,iteration_relaunch,1,0,x0, prior_fail_con,position,SLSQP_dial)         
            ...
            if relaunch_triggered == 1:
                ...
                defects_form = defect_energies(defects,chemical_potentials,number_of_defects,host_supercell,tab_correction,E_VBM,total_species,use_coul_correction,length,dielectric,v_M,1)       
                (nu_e_final,concentrations,dopant_concentration_sums,fail) = calc_fermi_dopopt(b,loop,defects,defects_form,number_of_defects,bandgap,temperature,def_statistics,nu_e,condband,valband,electron_method,hole_method,fixed_e_conc,fixed_p_conc,art_dop_conc,art_dop_charge,charge_convergence,val_band_min,val_band_max,cond_band_min,cond_band_max,seedname,entropies, fu_uc,electron_mass_eff,hole_mass_eff,unit_vol,charged_sys,number_dopants,bnds,dp_list,optimiser,log_diff_conv,function_tol,maxiter_dop,environment,prog_meter,prog_bar,num_iter,real_gas,xOpt)

                minimise_array = len(position)*[1]  #This array will hold the difference between each dopants current concentration and its target concentration
                ...
                if product > (2*log_diff_conv):
                    ...
                    chemical_potentials, dopants,iteration_relaunch, prior_fail_con=relaunch(b,loop,defects,dopants,chemical_potentials,number_of_defects,host_supercell,tab_correction,E_VBM,total_species,use_coul_correction,length,dielectric,v_M,bandgap,temperature,def_statistics,nu_e,condband,valband,electron_method,hole_method,fixed_e_conc,fixed_p_conc,art_dop_conc,art_dop_charge,charge_convergence,val_band_min,val_band_max,cond_band_min,cond_band_max,seedname,entropies, fu_uc,electron_mass_eff,hole_mass_eff,unit_vol,charged_sys, log_diff_conv,function_tol,maxiter_dop,environment,prog_meter,prog_bar,num_iter,real_gas,number_dopants,xOpt, bnds,dp_list,iteration_relaunch,2,product,x0, prior_fail_con,position,SLSQP_dial)         
...
def calc_opt_chem_pot_multidim(x, target_conc_list, b,loop,number_of_defects,host_supercell,tab_correction,E_VBM,total_species,use_coul_correction,length,dielectric,v_M,bandgap,temperature,def_statistics,nu_e,condband,valband,electron_method,hole_method,fixed_e_conc,fixed_p_conc,art_dop_conc,art_dop_charge,charge_convergence,val_band_min,val_band_max,cond_band_min,cond_band_max,seedname,entropies, fu_uc,electron_mass_eff,hole_mass_eff,unit_vol,charged_sys,number_dopants,position,key_list, bnds,dp_list,optimiser,environment,prog_meter,prog_bar,num_iter,log_diff_conv,function_tol,maxiter_dop,real_gas,iteration_relaunch):
    ...
    if environment == 'energy':
        print("..> SLSQP:",iteration_relaunch+1,",", iteration_slsqp, ", max(log10([target])-log10([present])):",max(minimise_array2),'        ', end="\r", flush=True)
   
    else:
        print("..> Calculating defect concentrations for",environment,prog_meter, "of", num_iter," [{0}]   ".format('#' * (prog_bar) + ' ' * (25-prog_bar)),"SLSQP:",iteration_relaunch+1,",", iteration_slsqp,",",max(minimise_array2),'       ', end="\r", flush=True)
...
def relaunch(b,loop,defects,dopants,chemical_potentials,number_of_defects,host_supercell,tab_correction,E_VBM,total_species,use_coul_correction,length,dielectric,v_M,bandgap,temperature,def_statistics,nu_e,condband,valband,electron_method,hole_method,fixed_e_conc,fixed_p_conc,art_dop_conc,art_dop_charge,charge_convergence,val_band_min,val_band_max,cond_band_min,cond_band_max,seedname,entropies, fu_uc,electron_mass_eff,hole_mass_eff,unit_vol,charged_sys, log_diff_conv,function_tol,maxiter_dop,environment,prog_meter,prog_bar,num_iter,real_gas,number_dopants ,x, bnds,dp_list,iteration_relaunch,fail_con,product,prior_chemical_potentials, prior_fail_con,position,SLSQP_dial):         
    ...
    if iteration_relaunch == 1000:
        dopant_fail(x, bnds,dp_list,fail_con)
    ...
    for i in np.arange(0, len(position), 1):
        ...
        if (iteration_relaunch ==2) and (product > ex_product):
            chemical_potentials[int(2*(position[i])+1)]= chemical_potentials[int(2*(position[i])+1)]-0.1*abs(bnds[i][1]-bnds[i][0])
            ...
            if (fail_con == 1) and (prior_fail_con != 2):
                ...
                elif (x[i] > chemical_potentials[int(2*(position[i])+1)]) or (x[i] < chemical_potentials[int(2*(position[i])+1)]):
                    chemical_potentials[int(2*(position[i])+1)]= chemical_potentials[int(2*(position[i])+1)] + multi_num*(iteration_relaunch/SLSQP_dial)

            elif (fail_con == 1) and (prior_fail_con == 2):   
                                                                             
                if x[i] > chemical_potentials[int(2*(position[i])+1)]:
                    chemical_potentials[int(2*(position[i])+1)] = chemical_potentials[int(2*(position[i])+1)] +1.1*abs(x[i] - chemical_potentials[int(2*(position[i])+1)])
                ...
                if (x[i] == bnds[i][0]):
                    ...
                    chemical_potentials[int(2*(position[i])+1)] = prior_chemical_potentials[i] +0.8*abs(x[i] - prior_chemical_potentials[i])

                elif x[i] < prior_chemical_potentials[i]:
                    chemical_potentials[int(2*(position[i])+1)] = prior_chemical_potentials[i] -0.8*abs(x[i] - prior_chemical_potentials[i])

            if (x[i] == chemical_potentials[int(2*(position[i])+1)]) and (bound_indicator == 0):
                chemical_potentials[int(2*(position[i])+1)] = chemical_potentials[int(2*(position[i])+1)]+ multi_num*(iteration_relaunch/5)
...
def calc_fermi_dopopt(b,loop,defects,defects_form,number_of_defects,bandgap,temperature,def_statistics,nu_e,condband,valband,electron_method,hole_method,fixed_e_conc,fixed_p_conc,art_dop_conc,art_dop_charge,charge_convergence,val_band_min,val_band_max,cond_band_min,cond_band_max,seedname,entropies, fu_uc,electron_mass_eff,hole_mass_eff,unit_vol,charged_sys,dopants_opt,bnds,dp_list,optimiser,log_diff_conv,function_tol,maxiter_dop,environment,prog_meter,prog_bar,num_iter,real_gas,x):
...
def print_defect_phases(plot_master,nametags,tag):
...
def phases_graphical_output(min_value,max_value,seedname,loop,host_name, dopant_xvar, volatile_element,stability):
    ...
    #colour choice
    colours =["#006e00","#b80058","#008cf9","#d163e6","#00bbad","#ebac23","#5954d6","#ff9287","#b24502","#878500","#00c6f8","#00a76c","#bdbdbd"]
    colourx = "black"
    defect_colours = []
    defect_lines = []
                   
    graphfile = str(seedname)+"_phases.p"
    outputfile = str(seedname)+"_phases.eps"
    resultfile = str(seedname)+".phases"
    
    with open(graphfile, 'w') as f:
        #Print header to file
        print("#GNUPLOT script for showing phase stability\n",file=f)
        print("set terminal postscript eps enhanced color font 'Helvetica,20'", file=f)
        print("set output \"",outputfile,"\"", sep="",file=f)
        print("set encoding iso_8859_1",file=f)
        ...
        print("set ylabel 'Stability w.r.t. ",host_name,"'\n",sep="", file=f)
        print("set xrange [",min_value,":",max_value,"]",sep="", file=f)
        ...
        while i < stability[0]:
            ...
            line_type = 1
            if i==0 :
                print("plot \"./",resultfile,"\" using 1:",i+2," with lines lt ",line_type," lw 2 lc rgb \"", colour,"\" ti \"",phase,"\",\\",sep="",file=f)
            else:
                print("\"./",resultfile,"\" using 1:",i+2," with lines lt ",line_type," lw 2 lc rgb \"", colour,"\" ti \"",phase,"\",\\",sep="",file=f)
            i+=1

def defect_phases_graphical_output(min_value,max_value,min_value_y, max_value_y,seedname,loop,loop2,host_name, dopant_xvar, volatile_element,stability, nametags,increment,tag):
    ...
    with open(graphfile, 'w') as f:
        #Print header to file
        print("#GNUPLOT script for showing defect phase diagram\n",file=f)
        print("set terminal postscript eps enhanced color font 'Helvetica,20'", file=f)
        print("set output \"",outputfile,"\"", sep="",file=f)
        print("set encoding iso_8859_1",file=f)
        ...
        if(loop2 == 0):
            ...
            print("set ylabel 'log_{10}[",dopant_xvar,"] (per ",host_name,")'",sep="",file=f)
        elif(loop2 == 3):
            print("set ylabel 'log_{10}[artificial dopant conc] (per ",host_name,")'",sep="", file=f)
        elif(loop2 == 4):
            print("set ylabel 'log_{10}P_{",dopant_xvar,"_{2}} /atm'",sep="",file=f)
        ...
        while i < len(nametags):
            ...
            if i<13: 
                ...
                colour = colourx
            if i==0 :
                print("plot \"./",resultfile,"\" using 1:2 with circles lc rgb \"", colour,"\" ti \"",name,"\",\\",sep="",file=f)
            else:
                print("\"./",resultfile,"\" using 1:2 with circles lc rgb \"", colour,"\" ti \"",name,"\",\\",sep="",file=f)
            i+=1
...
def looper(loop, b,constituents, chem_pot_method, temperature,entropies, constituent_entropies,entropy_data,number_of_defects,constituents_name_list,seedname,dopants, dopant_xvar ,art_dop_conc, volatile_element,gibbs_marker,gibbs_data,host_energy ):
    ...
    if (loop == 1):     #Temperature   
        ...
        if(gibbs_marker == 1):
            host_energy,constituents = calc_gibbs(gibbs_data,b,constituents_name_list,chem_pot_method,host_energy,constituents,seedname,1)
    ...
    return(constituents,temperature,entropies, constituent_entropies, environment,dopants, dopant_xvar,art_dop_conc, volatile_element,host_energy )
...
#Read in data
host_array,dopants,tasks,constituents,constituents_name_list,temperature,def_statistics,tab_correction,host_energy,chem_pot_method,host_supercell,use_coul_correction,length,dielectric,v_M,E_VBM,bandgap,condband,valband,electron_method,hole_method,fixed_e_conc,fixed_p_conc,art_dop_conc,art_dop_charge,loop,min_value,max_value,iterator,gnuplot_version,min_y_range,max_y_range,host_name,val_band_min,val_band_max,cond_band_min,cond_band_max,y_form_min,y_form_max,lines, entropy_marker, entropy_units,fu_uc,electron_mass_eff,hole_mass_eff,unit_vol,charge_convergence,potential_convergence,stability,scheme,stoichiometry,x_variable,real_gas,function_tol,SLSQP_dial ,maxiter_dop,y_variable,loop2, min_value_y, max_value_y, iterator_y,accomodate,gibbs_marker=inputs(seedname)
...
#Determine whether constituent temperature dependence is used
if(gibbs_marker == 1):
    gibbs_data = read_gibbs(seedname)
    #Perform a check on input data
    gibbs_check(gibbs_data,constituents_name_list,chem_pot_method,seedname)
    host_energy,constituents = calc_gibbs(gibbs_data,temperature,constituents_name_list,chem_pot_method,host_energy,constituents,seedname,1)     
...
if ('brouwer' in tasks):
    ...
    for b in np.arange(min_value, max_value+(iterator/2), iterator):
        ...
        #Loop assignment
        constituents,temperature,entropies, constituent_entropies, environment,dopants, dopant_xvar,art_dop_conc, volatile_element,host_energy= looper(loop, b,constituents, chem_pot_method, temperature,entropies, constituent_entropies,entropy_data,number_of_defects,constituents_name_list,seedname,dopants, dopant_xvar ,art_dop_conc, volatile_element,gibbs_marker,gibbs_data,host_energy )
...
if ('phases' in tasks):
    ...
    for b in np.arange(min_value, max_value+(iterator/2), iterator):
        ...
        prog_bar = round((prog_meter/num_iter)*25)
        print("..> Calculating phase stability for",environment,prog_meter, "of", num_iter," [{0}]   ".format('#' * (prog_bar) + ' ' * (25-prog_bar)), end="\r", flush=True)
        ...
        for i in np.arange(0,number_dopants, 1):            #Create record of *past* dopant concentrations. 
            i = int(i)
            current_conc = math.log10(dopants[6*i+4])
            dop_conc_list.append(current_conc)
        ...
        while indicator2 == 1:
            ...
            ## !!! Add a check for that makes sure phases added in input file !!!
            
            stability_printout,indicator2= stability_check(stability,chemical_potentials,indicator,b)

            phase_seperation = 1         #phase_seperation = 1 : remove instable phase



            if phase_seperation == 0:
                indicator2 =0

            if phase_seperation == 1:

                    if indicator2 ==1:  #indicator2 ==1 : there is an unstable phase   
                      
                         relative_stability_list =[]
                         for i in np.arange(0, stability[0], 1):
                            i =int(i)
                           
                            relative_stability_list.append(stability_printout[i][3])

                            
                                     
                         adjust_step +=1
                         
                         i= relative_stability_list.index(max(relative_stability_list))

                
                         phase_breakdown= stability[3*i+3]
                         stable_phases.append(phase_breakdown)
                   

                         element_list=[]
                         for j in np.arange(0, phase_breakdown[0], 1):
                             j =int(j)
                             element = phase_breakdown[2*j+1]
                             element_stoic = phase_breakdown[2*j+2]
                             if (element in dopants):
                                 element_list.append(element)
                                 element_list.append(element_stoic)

                         for k in np.arange(0,number_dopants, 1):
                             k = int(k)
                             dopant = dopants[6*k+1]
                             if (dopant in element_list):

                                

                                 if (dopant not in stable_elements):    #record number of phases that describe dopants that, at one time, are the most stable
                                     stable_elements.append(dopant)

                                     if (i not in stable_count):
                                         stable_count.append(i)
                                         



                                 
                                 dopant_stoic = element_list[element_list.index(dopant)+1]
                                 current_conc = math.log10(dopants[6*k+4])
                                 new_conc= current_conc-(dop_fac*dopant_stoic)

                             

                                 
                                 dopants[6*k+4] = 10**new_conc

                    if (indicator2 == 0) and (adjust_step != 0):
                        ...
                        for i in np.arange(0, stability[0], 1):
                            ...
                            relative_stability_list.append(stability_printout[i][3])
                            relative_stability_list_sorted = sorted(relative_stability_list,reverse=True)

                        for x in np.arange(0, len(stable_count), 1):

                            x =int(x)

                            print(relative_stability_list_sorted[x])

                            if (relative_stability_list_sorted[x]) < -0.01:     #overshoot

                                indicator2 = 1

                                

                                for p in np.arange(0, stability[0], 1):       #Get correct phase
                              

                                    i= relative_stability_list.index(relative_stability_list_sorted[x])
                                    phase_breakdown= stability[3*i+3]

                                    if phase_breakdown in stable_phases:
                                        break
                                ...
                                for k in np.arange(0,number_dopants, 1):
                                    ...
                                    if (dopant in element_list):
                                        ...
                                        current_conc = math.log10(dopants[6*k+4])
                                        new_conc= current_conc+(dop_fac*dopant_stoic)
                                        if new_conc >dop_conc_list[k]:
                                            new_conc = dop_conc_list[k]

                                        dopants[6*k+4] = 10**new_conc

                        dop_fac = 0.5*dop_fac
        print(relative_stability_list)
        print(dopants)

        phases = [b]
        for i in np.arange(0, stability[0], 1):
            i =int(i)
            phases.append(-1*stability_printout[i][3])
        ...
        final_concentrations2.append(concentrations)
        prog_meter+=1
    ...
    if ('group' in tasks):     
        ...
        (final_grouped_concs,group_list) = group(final_concentrations2,number_of_defects,defects,num_iter, stoichiometry)

        #Print the seedname.res file
        print_results(final_grouped_concs,seedname)
       
        #Generate Brouwer diagram
        graphical_output(number_of_defects,min_value,max_value,final_concentrations2,seedname,loop,gnuplot_version,min_y_range,host_name,group_list,electron_method,hole_method,dopants,host_array,1,conc_colour,electron_colour,hole_colour,scheme, dopant_xvar,stoichiometry,x_variable,total_species,volatile_element,charged_sys )
        
    else:

        #Print the seedname.res file
        print_results(final_concentrations2,seedname)

        #Generate Brouwer diagram
        graphical_output(number_of_defects,min_value,max_value,final_concentrations2,seedname,loop,gnuplot_version,min_y_range,host_name,defects,electron_method,hole_method,dopants,host_array,0,conc_colour,electron_colour,hole_colour,scheme, dopant_xvar, stoichiometry,x_variable,total_species,volatile_element,charged_sys )

    #Print the seedname.res file
    print_phases(final_phases,seedname)
    
    #Generate plot file
    phases_graphical_output(min_value,max_value,seedname,loop,host_name, dopant_xvar, volatile_element,stability)
...
if ('defect_phase' in tasks):
    ...
    for a in np.arange(min_value, max_value+(iterator/2), iterator):
        ...
        #Loop assignment
        constituents,temperature,entropies, constituent_entropies, environment,dopants, dopant_xvar,art_dop_conc, volatile_element,host_energy = looper(loop, a,constituents, chem_pot_method, temperature,entropies, constituent_entropies,entropy_data,number_of_defects,constituents_name_list,seedname,dopants, dopant_xvar ,art_dop_conc, volatile_element,gibbs_marker,gibbs_data,host_energy )

        prog_bar = round((prog_meter/num_iter)*25)
        print("..> Calculating defect concentrations for x-axis value",prog_meter, "of", num_iter," [{0}]   ".format('#' * (prog_bar) + ' ' * (25-prog_bar)), end="\r", flush=True)
        ...
        for b in np.arange(min_value_y, max_value_y+(iterator_y/2), iterator_y):
            ...
            #Loop assignment
            constituents,temperature,entropies, constituent_entropies, environment2,dopants, dopant_xvar,art_dop_conc, volatile_element,host_energy = looper(loop2, b,constituents, chem_pot_method, temperature,entropies, constituent_entropies,entropy_data,number_of_defects,constituents_name_list,seedname,dopants, dopant_xvar ,art_dop_conc, volatile_element,gibbs_marker,gibbs_data,host_energy )
            ...
            if ('dopant' in tasks):
                #For plotting accomodation mechanism of 1 dopant that is assigned in input file with 'Accomodate' tag. 
            
                for i in np.arange(0,dopants[0], 1):
                    i = int(i)
                    if dopants[6*i+1] == accomodate:
                        signal_i = i 
                ...
                max_stable =0 
                indicator_accomodate = 0 
                if ('stability' in tasks):
                    if indicator == 1:
                        for i in np.arange(0,len(stability_printout), 1):
                            i=int(i)
                            if (stability_printout[i][3] > max_stable) and (accomodate in stability[3*i+3]):
                                indicator_accomodate = 1
                                max_i = i
                                max_stable = stability_printout[i][3]
                        if indicator_accomodate == 1:
    ...
    print_defect_phases(all_plot_master,A_nametags,1)

    #Generate defect phases diagram
    defect_phases_graphical_output(min_value,max_value,min_value_y, max_value_y,seedname,loop,loop2,host_name, dopant_xvar, volatile_element,stability, A_nametags,iterator,1)

    if ('dopant' in tasks):
        #Print the defect_phases files
        print_defect_phases(dopant_plot_master,D_nametags,2)

        #Generate defect phases diagram
        defect_phases_graphical_output(min_value,max_value,min_value_y, max_value_y,seedname,loop,loop2,host_name, dopant_xvar, volatile_element,stability, D_nametags,iterator,2)

#print stability readout, if requested
if ('stability' in tasks):
    print("\n>>> Task:'stability':")
    if indicator == 1:
        print("<!> WARNING: Stability check for supplied compounds has found stable compounds. See"," ",seedname,".output for details",sep="" )
    else:
        print("..> Stability check for supplied compounds complete. No stable compounds found, see"," ",seedname,".output for details",sep="" )
